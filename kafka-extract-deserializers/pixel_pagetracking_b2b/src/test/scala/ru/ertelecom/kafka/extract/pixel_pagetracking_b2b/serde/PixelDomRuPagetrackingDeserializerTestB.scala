...
class PixelDomRuPagetrackingDeserializerBTest extends FunSuite {

    test("successPixelDomRuPagetrackingDeserializer/FullPacket") {
        val deserializer = new PixelDomRuPagetrackingDeserializerB(null)
    ...
    test("feature: BIGD-873: # in url") {
        ...
        assert(Some("Yandex_Dzen") == p.get.utm_source)
    }


    test("check b2b cookie: all") {
        val deserializer = new PixelDomRuPagetrackingDeserializerB(null)
        val testValue = """2020-09-09T13:26:24+00:00	95.37.62.106	/statb.gif?v=1&_v=j85&a=1482534144&t=pageview&_s=1&dl=https%3A%2F%2Flkb2b.domru.ru%2Fprivilege-program&ul=ru&de=UTF-8&dt=%D0%9F%D1%80%D0%BE%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0%20%D0%BF%D1%80%D0%B8%D0%B2%D0%B8%D0%BB%D0%B5%D0%B3%D0%B8%D0%B9%20%7C%20%D0%9B%D0%B8%D1%87%D0%BD%D1%8B%D0%B9%20%D0%BA%D0%B0%D0%B1%D0%B8%D0%BD%D0%B5%D1%82%20B2B%20%D0%94%D0%BE%D0%BC.ru&sd=24-bit&sr=1440x900&vp=1100x749&je=1&_u=QCEAgAAB~&jid=1722119843&gjid=268984860&cid=410825048.1599619412&uid=nn%D0%9510939585&tid=UA-42532108-1&_gid=1825402466.1599619412&gtm=2wg8q1MVZV2&cd1=nn%D0%9510939585&cd2=410825048.1599619412&cd3=nn&z=340799795	Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_6) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.1.2 Safari/605.1.15	uid=		p_uid=589CBABC5441585FCE2FEC1802EA802E	http_ref=https://lkb2b.domru.ru/privilege-program	nn	nn%D0%9510939585"""
        val p = deserializer.parseTsv(testValue.getBytes(Charset.forName("UTF-8")))

        assert(p.nonEmpty)
        assert(Some("nn") == p.get.citydomain)
        //assert(Some("E10939585") == p.get.client_id_b2b)
        assert("Е10939585" == URLDecoder.decode("%D0%9510939585", "UTF-8"))
    }

    test("check b2b cookie: spb") {
        val deserializer = new PixelDomRuPagetrackingDeserializerB(null)
